...
describe('coordinate parser', function() {
  it('should throw if passed an incorrect format', function() {
    expect(function() {parseCoord.parse('X1Y1', {})}).to.throw(/format undefined/)
  })

  it('should parse properly with leading zero suppression', function() {
    FORMAT.zero = 'L'
    expect(parseCoord.parse('X10', FORMAT)).to.eql({x: .01})
    expect(parseCoord.parse('Y15', FORMAT)).to.eql({y: .015})
    expect(parseCoord.parse('I20', FORMAT)).to.eql({i: .02})
    expect(parseCoord.parse('J-40', FORMAT)).to.eql({j: -.04})
    expect(parseCoord.parse('X1000Y-2000I3J432', FORMAT)).to.eql({x: 1, y: -2, i: .003, j: .432})
  })
  ...
  it('should parse properly with trailing zero suppression', function() {
    ...
    expect(parseCoord.parse('X0001Y-0002I3J432', FORMAT)).to.eql({x: .01, y: -.02, i: 30, j: 43.2})
  })
  ...
  it('should parse properly with explicit decimals mixed in', function() {
    ...
    expect(parseCoord.parse('I20', FORMAT)).to.eql({i: .02})
    expect(parseCoord.parse('J-40', FORMAT)).to.eql({j: -.04})
    expect(parseCoord.parse('X1.1Y-2.02I3.3J43.2', FORMAT)).to.eql({x: 1.1, y: -2.02, i: 3.3, j: 43.2})
  })
